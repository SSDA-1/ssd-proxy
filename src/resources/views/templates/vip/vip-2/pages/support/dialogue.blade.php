...
@section('content')
    ...
    <div class="wrap-support">
        <div class="pastAppeals">
            <div class="warp-Appeals" id="element">
                ...
                @foreach ($supportsList as $item)
                    <div class="cardAppeals {{$id == $item->id ? 'action' : ''}}">
                        <a href="/support/{{$item->id}}" style="text-decoration: unset;"><h3>Обращение
            ...
            <div class="main_btn" id="dialog_btn">Новое обращение</div>
        </div>
        <div class="bodyDialog" id="bodyDialog">
            {{--{{ dd($support) }}--}}

            @if($support)
                <div class="topLine">Обращение #{{$id}} @if ($support->status == 0)
                        <span style="color:green;">Открыт</span>
                    @else
                        <span style="color:red;">Закрыт</span>
                    @endif</div>
