...
<style>
    ...
    .wrap-title {width: var(--width-1200); display: flex; margin-top:50px;}
    ...
    .wrap-support .pastAppeals h2 {margin-bottom:10px;}
    ...
    .wrap-support .pastAppeals .warp-Appeals {max-height: 395px; overflow: auto; margin-bottom: 5px; padding-right: 10px;}
...
<div class="wrap-support">
    <div class="pastAppeals">
        <div class="warp-Appeals" id="element">
            <h2>Прошлые обращения</h2>
            {{-- {{print($supportsList)}} --}}
            @foreach ($supportsList as $item)
                <div class="cardAppeals ">
                    <a href="/support/{{$item->id}}" style="text-decoration: unset;"><h3>Обращение #{{$item->id}} @if ($item->status == 0) <span style="color:green;">Открыт</span> @else <span style="color:red;">Закрыт</span> @endif<span>{{$item->lastsuppmassage ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $item->lastsuppmassage->updated_at)->format('d.m.Y') : '' }}</span></h3>
